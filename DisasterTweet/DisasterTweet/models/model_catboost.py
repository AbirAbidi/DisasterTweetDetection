...
X = data['processed_text_no_stopwords']  
y = data['target']  
...
X_train = vectorizer.fit_transform(X_train)  # This will be a sparse matrix
X_test = vectorizer.transform(X_test)  # This will be a sparse matrix
...
model_cat = CatBoostClassifier(iterations=100, depth=6, learning_rate=0.1)
# Entraîner le modèle sans spécifier 'cat_features'
model_cat.fit(X_train_dense, y_train)
#y_pred = model_cat.predict(X_test_dense)
import pickle
with open('catboost.pkl', 'wb') as model_file:
    pickle.dump((model_cat, vectorizer), model_file)