...
"""STAKING logistic regression random forest decision tree"""
X = data['processed_text']
y = data['target']
X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.3, random_state=42)
tfidf_vectorizer = TfidfVectorizer(max_features=1000)  # Limiter à 1000 features
X_train_tfidf = tfidf_vectorizer.fit_transform(X_train)
X_test_tfidf = tfidf_vectorizer.transform(X_test)
# Définir les modèles de base (Level 0)
models = [
    ('lr', LogisticRegression(max_iter=1000)),       # Régression logistique
    ('rf', RandomForestClassifier(n_estimators=50)), # Forêt aléatoire
    ('dt', DecisionTreeClassifier())                 # Arbre de décision
]
...
import pickle
with open('stacking.pkl', 'wb') as model_file:
    pickle.dump((stacking_model, tfidf_vectorizer), model_file)
